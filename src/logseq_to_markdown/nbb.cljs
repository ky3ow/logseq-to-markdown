...
(defn good-path
  [graph-name options]
  (config/set options)
  (if (config/entry :generate-database)
    (let [generated-graph-name (graph/generate-transit graph-name)]
      (println "Generated transit file for graph:")
